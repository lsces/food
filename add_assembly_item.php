...
if( !empty( $_REQUEST['fAddComponent'] ) ) {
	...
	if( $title === '' ) {
		...
		// exact-match fallback genuinely ambiguous for several real components).
		// Still verified against content_type_guid rather than trusted blindly —
		// a tampered or stale id falls straight through to the exact-title
		// fallback instead.
