		...
		if( in_array( $mealType, $taken, true ) ) {
			...
			if( $new->store( $pHash ) ) {
				...
				foreach( $sourceItems as $item ) {
					// Applied before addItem() itself — mirrors add_assembly_item.php's
					// own REM decrement (a copied meal eats ingredients the same as one
					// built by hand), clamped by adjustComponentRem() (empty pantry, or
					// its dust threshold — see that method's docblock). The actual delta
					// applied is passed into addItem() so it can stash it on the line
					// (xkey_ext) for an accurate later restock — see addItem()'s
					// $pRemRestockAmount docblock.
					$actualDelta = $movement->adjustComponentRem( (int)$item['component_content_id'], -(float)$item['quantity'] );
					$new->addItem(
						$mealType, (int)$item['component_content_id'], (int)$item['quantity'], (int)$item['xorder'],
						null, null, -$actualDelta
					);
				}
